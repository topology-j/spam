...
type Tab = 'reports' | 'spam-check' | 'rules' | 'chats' | 'reviews' | 'profile'
...
// ── AI 스팸 판별 채팅 ─────────────────────────────────
interface Msg { id: number; role: string; message: string; is_spam: number | null }
const chatMessages = ref<Msg[]>([])
const chatInput = ref('')
const chatLoading = ref(false)
const chatBody = ref<HTMLElement | null>(null)
const counselorFileInput = ref<HTMLInputElement | null>(null)
let nextMsgId = 0
...
async function sendChat() {
  ...
  chatInput.value = ''
  const ta = document.querySelector('.spam-check-input') as HTMLTextAreaElement
  if (ta) ta.style.height = 'auto'
  chatMessages.value.push({ id: nextMsgId++, role: 'user', message: text, is_spam: null })
  ...
  try {
    ...
    chatMessages.value.push({ id: nextMsgId++, role: 'ai', message: data.reply, is_spam: data.is_spam ? 1 : 0 })
  } catch {
    chatMessages.value.push({ id: nextMsgId++, role: 'ai', message: '서버에 연결할 수 없습니다.', is_spam: null })
  } finally {
...
async function uploadCounselorFile(e: Event) {
  const file = (e.target as HTMLInputElement).files?.[0]
  if (!file || chatLoading.value) return
  chatLoading.value = true
  chatMessages.value.push({ id: nextMsgId++, role: 'user', message: `📎 ${file.name} 업로드`, is_spam: null })
  await nextTick()
  chatBody.value?.scrollTo({ top: chatBody.value.scrollHeight, behavior: 'smooth' })
  try {
    ...
    chatMessages.value.push({ id: nextMsgId++, role: 'ai', message: err.message || '파일 처리 실패', is_spam: null })
  } finally {
    chatLoading.value = false
    if (counselorFileInput.value) counselorFileInput.value.value = ''
    await nextTick()
    chatBody.value?.scrollTo({ top: chatBody.value.scrollHeight, behavior: 'smooth' })
  }
}
...
onMounted(() => { loadReports(); loadProfile() })
</script>
...
<template>
  <div class="page">
    ...
    <div class="layout">
          ...
          <div v-else class="detail-content">
            ...
            <div class="note-section" style="margin-top:14px">
              <label class="note-label">
                스팸 키워드 등록
                <span class="note-hint">(한 줄에 하나씩 · 처리 완료 시 적용)</span>
              </label>
      ...
      <!-- AI 스팸 판별 탭 -->
      <div v-else-if="activeTab === 'spam-check'" class="full-panel">
        <div class="panel-header">
          <div>
            <h2 class="panel-title">AI 스팸 판별</h2>
            <p class="panel-desc">단어·문장을 입력하면 스팸 여부를 즉시 분석합니다</p>
          </div>
        ...
        <div class="sc-chat-body" ref="chatBody">
          <div v-if="!chatMessages.length" class="empty-state-sm" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px">
          ...
          <TransitionGroup name="msg" tag="div" class="sc-messages">
            <div v-for="msg in chatMessages" :key="msg.id" class="sc-msg-row" :class="msg.role">
              <div v-if="msg.role === 'ai'" class="sc-ai-avatar">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              </div>
              <div class="sc-bubble" :class="[msg.role, { 'bubble-spam': msg.is_spam === 1, 'bubble-ham': msg.is_spam === 0 }]">
                <span v-if="msg.is_spam === 1" class="sc-label spam">스팸 감지</span>
                <span v-else-if="msg.is_spam === 0" class="sc-label ham">정상</span>
                {{ msg.message }}
              </div>
            </div>
          </TransitionGroup>
          <div v-if="chatLoading" class="sc-msg-row ai">
            <div class="sc-ai-avatar"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg></div>
            <div class="sc-bubble ai loading-bubble"><span class="dot"/><span class="dot"/><span class="dot"/></div>
          </div>
        </div>
        <div class="sc-input-bar">
          <input ref="counselorFileInput" type="file" accept=".pdf,.txt,.eml" style="display:none" @change="uploadCounselorFile" />
          <button class="sc-upload-btn" :disabled="chatLoading" @click="counselorFileInput?.click()" title="파일 업로드 (PDF, TXT, EML)">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/>
            </svg>
          </button>
          <textarea
            v-model="chatInput" class="spam-check-input"
            placeholder="단어 또는 문장을 입력하거나 파일을 업로드하세요... (Enter 전송)"
            rows="1" @keydown="onChatKey" @input="autoResizeChat"
          />
          <button class="sc-send-btn" :disabled="!chatInput.trim() || chatLoading" @click="sendChat">
      ...
      <!-- 스팸 규칙 탭 -->
      <div v-else-if="activeTab === 'rules'" class="full-panel">
        <div class="panel-header">
          <div>
      ...
      <div v-else-if="activeTab === 'chats'" class="full-panel">
        <div class="panel-header">
          <div>
            <h2 class="panel-title">스팸 확인 로그</h2>
            <p class="panel-desc">사용자의 AI 스팸 판별 내역을 확인합니다</p>
          </div>
          <div style="display:flex;align-items:center;gap:10px">
            <div class="filter-pills">
              <button :class="['pill', { active: chatLogFilter === 'all' }]" @click="chatLogFilter = 'all'">전체 {{ chatLogs.length }}</button>
              <button :class="['pill', 'pill-spam', { active: chatLogFilter === 'spam' }]" @click="chatLogFilter = 'spam'">스팸 {{ spamCount }}</button>
              <button :class="['pill', 'pill-ham', { active: chatLogFilter === 'ham' }]" @click="chatLogFilter = 'ham'">정상 {{ chatLogs.length - spamCount }}</button>
            </div>
        ...
        <div class="chat-log-list">
          <div v-if="!filteredChatLogs.length" class="empty-state-sm">{{ chatLogFilter === 'spam' ? '스팸으로 감지된 내역이 없습니다.' : chatLogFilter === 'ham' ? '정상 판별 내역이 없습니다.' : '채팅 기록이 없습니다.' }}</div>
          <div v-for="log in filteredChatLogs" :key="log.id" :class="['chat-log-item', log.role, { 'log-spam': log.is_spam === 1 }]">
            <div class="log-meta">
              <span class="log-user">{{ log.username }}</span>
              <span :class="['log-role-badge', log.role]">{{ log.role === 'user' ? '사용자' : 'AI' }}</span>
              <span v-if="log.is_spam === 1" class="badge-spam">스팸 감지</span>
